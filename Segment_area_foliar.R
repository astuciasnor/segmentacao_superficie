...
# Carregando imagens ------------------------------------------------------
  ...
  im <- readImage("images/area_foliar/imagem.jpg")
  plot(im)
  ref <- readImage("images/area_foliar/Referencia.jpg")
  plot(ref)
  fundo <- readImage("images/area_foliar/fundo_acerola.jpg")
  plot(fundo)
  folhas <- readImage("images/area_foliar/folhas.jpg")
  plot(folhas)
...
# Vamos colocar uma matriz sobre a outra
  Mat1 <- rbind(cbind(mfundo, 1), # Fará de preto o fundo
                cbind(mfolhas,0), # Objeto de interesse, será branco
...
# Criando uma terceira matriz para separar a folha do fundo e da ref --------------
  ...
  summary(ID2) # Observe que pega 30 objetos (29 folhas e a ref.)
...
# Objeto de referencia (cartão vermelho): 46.75 cm²
  ...
  plot(im)
  text(Coord[,1],Coord[,2],round(AreaCor,2 ),col = "yellow", cex=0.8)
